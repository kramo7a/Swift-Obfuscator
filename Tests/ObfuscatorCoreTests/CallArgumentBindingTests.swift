...
@Test func parameterCallArgumentBindingsResolveDefaultsVariadicsAndTrailingClosures() throws {
    ...
    let semanticIndex = SemanticIndex(snapshot: snapshot, obfuscationRoots: [file])
    ...
    let bindingIndex = CallArgumentBinding.Index(
        signatures: semanticIndex.callableSignatures,
        parametersByUSR: parameterSyntax.parametersByUSR,
        callSiteSyntax: callSiteSyntax
    )

    let sendAnchor = CallSiteSyntax.Anchor(
        callableUSR: send.usr,
        location: IndexSnapshot.Location(
            path: file.path,
            line: 7,
            utf8Column: utf8Column(of: "send", in: lines[6])
        )
    )
    let sendBindings = try #require(bindingIndex.callsByAnchor[sendAnchor])
    #expect(
    ...
    let finishAnchor = CallSiteSyntax.Anchor(
    ...
    let finishBindings = try #require(bindingIndex.callsByAnchor[finishAnchor])
    ...
    let summary = bindingIndex.report
    #expect(summary.signatureCountWithNamedExternalLabels == 3)
    #expect(summary.labeledParameterCount == 9)
    ...
    #expect(summary.signatureCountWithAllIndexedCallsBound == 2)
    #expect(summary.namedParameterCountInSignaturesWithAllIndexedCallsBound == 7)
    #expect(summary.signatureCountWithoutIndexedCalls == 0)
    #expect(summary.namedParameterCountInSignaturesWithoutIndexedCalls == 0)
    #expect(
