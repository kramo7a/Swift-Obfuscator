...
public final class IndexSnapshotReader {
    public enum Error: LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .missingIndexStore(let url):
                return
                    "Index store does not exist at \(url.path). Make sure xcodebuild completed with COMPILER_INDEX_STORE_ENABLE=YES."
            case .missingIndexDatabase(let url):
                return
                    "Index database does not exist at \(url.path). Run once without --reuse-index."
            case .missingIndexStoreLibrary:
    ...
    public func read(
        ...
        guard fileManager.fileExists(atPath: storePath.path) else {
            throw IndexSnapshotReader.Error.missingIndexStore(storePath)
        }

        if reuseExistingDatabase {
            guard fileManager.fileExists(atPath: databasePath.path) else {
                throw IndexSnapshotReader.Error.missingIndexDatabase(databasePath)
            }
        } else {
        ...
        let swiftFiles = try SwiftSourceFinder.swiftFiles(
            in: sourceRoot,
            excluding: excludedSourceRoots,
            fileManager: fileManager
        )
        var symbolsByUSR: [String: IndexSnapshot.Symbol] = [:]
        var occurrences: Set<IndexSnapshot.Occurrence> = []
        ...
        for file in swiftFiles {
            for candidate in SourcePathNormalizer.indexPathCandidates(for: file) {
                ...
                for symbol in fileSymbols {
                    symbolsByUSR[symbol.usr] = IndexSnapshot.Symbol(symbol)
                }
                for occurrence in fileOccurrences {
                    let record = IndexSnapshot.Occurrence(occurrence)
        ...
        return IndexSnapshot(
            ...
            occurrences: occurrences.sorted { lhs, rhs in
                (lhs.path, lhs.line, lhs.utf8Column, lhs.usr, lhs.rolesRaw) < (
                    rhs.path, rhs.line, rhs.utf8Column, rhs.usr, rhs.rolesRaw
                )
            }
        )
    }
    ...
    private func locateIndexStoreLibrary() throws -> URL {
        if let swiftPath = try? runner.run(
            executable: "/usr/bin/xcrun", arguments: ["--find", "swift"]
        ).stdout.trimmedNonEmpty {
        ...
        throw IndexSnapshotReader.Error.missingIndexStoreLibrary
    }
}

public enum SwiftSourceFinder {
    public static func swiftFiles(
        ...
        for case let url as URL in enumerator {
            ...
            if isDirectory,
                shouldSkipDirectory(name) || isExcludedDirectory(url, excludedRoots: excludedRoots)
            {
    ...
    private static func shouldSkipDirectory(_ name: String) -> Bool {
        [".git", ".swiftpm", ".obfuscator", "Derived", "DerivedData", "Index.noindex"].contains(
            name)
            || name == ".build"
