...
/// Semantic declaration metadata derived from the compiler index for one snapshot.
///
/// RenameEligibilityAnalyzer consumes this value instead of reconstructing Swift ownership
/// or declaration nesting from source braces. Source text remains responsible
/// only for lexical token/range validation and the limited syntax information that the
/// index does not encode.
public struct SemanticIndex: Sendable {
    ...
    public let callableSignatures: [CallableSignature]
    public let callableReport: CallableSignature.Report

    let symbolsByUSR: [String: IndexSnapshot.Symbol]
    ...
    public init(
        ...
        self.callableSignatures = callableSignatures
    ...
    public init(snapshot: IndexSnapshot, obfuscationRoots: [URL]) {
        ...
        var protocolUSRsByCallableAndSite: [String: [DeclarationLineSite: Set<String>]] = [:]
        ...
        var runtimeSeeds = Set(
            symbolsByUSR.values.compactMap { symbol -> String? in
        ...
        for occurrence in snapshot.occurrences {
            ...
            for relation in occurrence.relations {
                let occurrenceIsSynthetic = IndexSymbolName.isSyntheticAccessor(
                    occurrence.symbol.name)
                ...
                let isDispatchRelation =
                    Self.isOverrideDispatchKind(occurrence.symbol.kind)
        ...
        let localNominalUSRs = Set(
            selectedDeclarationUSRs.compactMap { usr -> String? in
                guard let symbol = symbolsByUSR[usr],
                    localNominalKinds.contains(symbol.kind)
                else {
                    return nil
                }
                return usr
            })
        let localProtocolUSRs = Set(
            localNominalUSRs.filter {
                symbolsByUSR[$0]?.isKind(.protocol) == true
            })

        let protocolRequirementUSRs = Set(
            selectedDeclarationUSRs.filter { usr in
        ...
        let externalExtensionUSRs = Set(
            selectedExtensionUSRs.filter { extensionUSR in
        ...
        let decodingParameterProtocolUSR =
            commonDecodingParameterProtocolUSRs.count == 1
        ...
        let encodingParameterProtocolUSR =
            commonEncodingParameterProtocolUSRs.count == 1
        ...
        let serializationSensitiveOwnerUSRs =
            decodingSensitiveOwnerUSRs
            .union(encodingSensitiveOwnerUSRs)
        let explicitCodingKeysEnumUSRs = Set(
            selectedDeclarationUSRs.filter { usr in
                symbolsByUSR[usr]?.isKind(.enum) == true && symbolsByUSR[usr]?.name == "CodingKeys"
            })
        let explicitCodingKeysOwnerUSRByEnumUSR = Dictionary(
        ...
        let callableSignatures = CallableSignature.Builder.makeSignatures(
            snapshot: snapshot,
            rootPaths: rootPaths,
            symbolsByUSR: symbolsByUSR,
        ...
        self.callableSignatures = callableSignatures
        self.callableReport = CallableSignature.Report(
    ...
    func qualifiedNominalOwnerUSRs(for ownerUSR: String) -> [String]? {
        ...
        while true {
            ...
            guard owners.count == 1, let rawOwnerUSR = owners.first,
                ...
                visited.insert(nominalOwnerUSR).inserted
            else {
                return nil
            }
    ...
    private static func isRuntimeSymbol(_ symbol: IndexSnapshot.Symbol) -> Bool {
    ...
    private static func hasRuntimeProperty(_ symbol: IndexSnapshot.Symbol) -> Bool {
    ...
    private static func isOverrideRelation(_ relation: IndexSnapshot.Relation) -> Bool {
        relation.hasRole(.overrideOf) || relation.hasRole(.baseOf)
    }

    private static func isPropertyWrapperDerivedName(_ derivedName: String, from parentName: String)
        -> Bool
    {
    ...
    private static func isSerializationImplementationSymbol(_ symbol: IndexSnapshot.Symbol) -> Bool
    {
