...
public struct ProjectBuildOptions: Sendable {
...
public enum ProjectBuilderError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .multipleContainers(let containers):
            return "Multiple Xcode project/workspace containers found: \(containers.joined(separator: ", ")). Pass extra xcodebuild arguments after --."
        }
    }
}
...
public final class ProjectBuilder {
    ...
    public func build(_ options: ProjectBuildOptions) throws -> ProjectBuildResult {
        let root = options.projectRoot.standardizedFileURL
        let scheme = try options.scheme ?? inferScheme(projectRoot: root)
        try fileManager.createDirectory(at: options.derivedDataPath, withIntermediateDirectories: true)
        ...
        return ProjectBuildResult(
            ...
            indexStorePath: options.derivedDataPath.appendingPathComponent("Index.noindex/DataStore", isDirectory: true)
        )
    }
    ...
    public func inferScheme(projectRoot: URL) throws -> String {
        ...
        guard let scheme = schemes.sorted().first else {
            throw ProjectBuilderError.noSchemeFound(projectRoot)
        }
        return scheme
    }
    ...
    private func containerArguments(projectRoot: URL) throws -> [String] {
        ...
        if workspaces.count > 1 {
            throw ProjectBuilderError.multipleContainers(workspaces.map(\.lastPathComponent))
        }
        if projects.count == 1 {
            return ["-project", projects[0].path]
        }
        if projects.count > 1 {
            throw ProjectBuilderError.multipleContainers(projects.map(\.lastPathComponent))
        }
        return []
    }
}
